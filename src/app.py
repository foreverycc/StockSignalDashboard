...
if selected_file:
    ...
    try:
        ...
        st.sidebar.write(f"Preview ({len(stocks)} stocks):")
        st.sidebar.write(", ".join(stocks[:10]) + ("..." if len(stocks) > 10 else ""))
...
# First table view with tabs 1-2
...
with tab1:
    ...
    if df is not None:
        ...
        if 'date' in df.columns:
            ...
            selected_dates = st.multiselect("Filter by date:", dates, default=dates[:5] if len(dates) > 5 else dates)
            if selected_dates:
...
# Second table view with tabs 3-5
...
with tab4:
    ...
    if df is not None and 'best_intervals' in message:
        st.write(f"Showing results from: {message}")
...
with tab5:
    ...
    if df is not None and 'custom_detailed' in message:
        st.write(f"Showing results from: {message}")
