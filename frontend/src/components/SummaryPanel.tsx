...
const Fetching1234ChartView = ({ row, type, runId, onClose }: { row: any, type: 'bull' | 'bear', runId: number | undefined, onClose: () => void }) => {
    ...
    const detailedRows = useMemo(() => {
        ...
        return intervals.map((interval: string) => {
            ...
            if (match) {
                // Determine active subtab for DetailedChartRow to pick correct color/logic
                // If it's 1234 signal, we use '1234' subtab logic
                return { ...match, ...extractBestMetrics(match) };
            }
...
export const SummaryPanel: React.FC<SummaryPanelProps> = ({ runId }) => {
    ...
    // 3. Detailed data for calculating average returns
    ...
    // Top Table for High Return Opportunities (existing)
        ...
        const sorted = useMemo(() => {
            ...
            const calculateAvgReturn = (row: any) => {
                ...
                for (const interval of intervalKeys) {
                    ...
                    if (matchingRow) {
                        // Use extractBestMetrics to get the best return from potentially multiple strictness levels
            ...
            // detailedData is already available in scope from props
            ...
            const filtered = [...data]
                ...
                .map(row => {
                    ...
                    if (calculatedReturn === null) {
                        // calculatedReturn should not be null now as we return 0
                    }
            ...
            // Sort by return if available, otherwise by date

            return filtered
                .sort((a, b) => {
                    // If both have returns, sort by return
