...
from app.logic.utils import (
    ...
    save_breakout_candidates,
    save_breakout_candidates_summary,
    save_results,
    save_breakout_candidates_1234,
    save_breakout_candidates_5230,
    save_mc_breakout_candidates_1234,
    save_mc_breakout_candidates_5230
)
...
def analyze_stocks(file_path, end_date=None):
    ...
    with Pool(num_processes) as pool:
        ...
        # We can't easily track progress with pool.map directly in a way that updates the job status
        # But we can log start/end
        
        # Chunking for better logging?
        chunk_size = 10
    ...
    output_base = os.path.basename(file_path).split('.')[0]
    ...
    logger.info(f"Aggregated {len(cd_results_1234)} CD 1234 breakout results and {len(cd_results_5230)} CD 5230 breakout results")
    logger.info(f"Aggregated {len(mc_results_1234)} MC 1234 breakout results and {len(mc_results_5230)} MC 5230 breakout results")
    if failed_tickers:
    ...
    save_results(cd_results_1234, output_file_1234)
    ...
    save_results(cd_results_5230, output_file_5230)
